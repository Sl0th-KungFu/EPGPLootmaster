...
function LootMasterML:GetFrame()
    ...
    local mainframe = LootMaster:CreateFrame("Frame","LootMasterMLMainFrame",UIParent)
    ...
    local frame = LootMaster:CreateFrame("Frame","LootMasterMLFrame",mainframe)
    ...
    local extralootframe = LootMaster:CreateFrame("Frame", "LootMasterMLFrameExtraLoot", frame)
    ...
    local titleFrame = LootMaster:CreateFrame("Frame", "MLMainTitleFrame", mainframe)
	...
	titletext:SetText( string.format("EPGPLootMaster %s by Bushmastert <MC Swimming Team> - Mirage Raceway EU", self:GetVersionString() ) )
    ...
    local equipHeaderFrame = LootMaster:CreateFrame("Frame", nil, frame)
    ...
    local tbGPValueFrame = LootMaster:CreateFrame("Frame", nil, frame)
...
function LootMasterML:OnCandidateRowRightClick( candidate, link, row )
    ...
    ToggleDropDownMenu(1, nil, self.CandidateDropDown, self.titleFrame, 0, 0);
end
...
function LootMasterML:ShowRaidInfoLookup()
    ...
    if not frame then
        ...
        frame = LootMaster:CreateFrame("Frame","LootMasterUIFrame",UIParent)
        --#region Setup main masterlooter frame
        frame:Hide();
        frame:SetWidth(700)
        frame:SetHeight(420)
        ...
        local titleFrame = LootMaster:CreateFrame("Frame", "nil", frame)
        --#region Setup main frame title
        titleFrame:SetBackdrop({
            bgFile = "Interface\\ChatFrame\\ChatFrameBackground",
            edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
        ...
        local titletext = titleFrame:CreateFontString(nil,"OVERLAY","GameFontNormalSmall")
        titletext:SetPoint("CENTER",titleFrame,"CENTER",0,1)
        titletext:SetText( string.format("EPGPLootMaster %s by Bushmastert <MC Swimming Team> - Mirage Raceway EU", LootMaster:GetVersionString() ) )
        frame.titleFrame = titleFrame
        --#endregion
		...
		local tbWhisperFrame = LootMaster:CreateFrame("Frame", nil, frame)
