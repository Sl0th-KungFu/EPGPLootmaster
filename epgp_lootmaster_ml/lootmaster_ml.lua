...
function LootMasterML:OnInitialize()
    ...
    self:RegisterEvent("GROUP_ROSTER_UPDATE",           "GROUP_UPDATE");
...
function LootMasterML:GetEPGP( player )
    ...
    if CEPGP_getEPGP then 
        local ep, gp = CEPGP_getEPGP(LootMaster.Ambiguate(player, "none"))
        return ep, gp, nil, CEPGP.GP.Min, player;
    end
...
function LootMasterML:CHAT_MSG_LOOT( event, message )
    ...
    if lootType==LootMaster.LOOTTYPE.GP and lootGP~=0 then
        ...
        elseif CanEditOfficerNote() and CEPGP_addGP then
            -- CEPGP Support
            local _,_,itemID = strfind(sLink, 'Hitem:(%d+)');
            print(LootMaster.Ambiguate(tostring(sPlayer), "none") .. "," .. lootGP .. "," .. itemID .. "," .. tostring(sLink))
            CEPGP_addGP(LootMaster.Ambiguate(tostring(sPlayer), "none"), lootGP, itemID, tostring(sLink) or 'nil', nil, nil)
        else
...
function LootMasterML:OPEN_MASTER_LOOT_LIST()
	...
	-- local lootIcon, lootName, lootQuantity, currencyID, rarity = GetLootSlotInfo(LootFrame.selectedSlot);
	local _, lootName, lootQuantity, _, rarity = GetLootSlotInfo(LootFrame.selectedSlot);
    ...
    -- Auto announce?
    self:Debug( format( 'Rarity check for auto announce item:%s aa>=%s', tostring(rarity), tostring(LootMaster.db.profile.auto_announce_threshold) ) )    

    local autoAnnounce = rarity or 0 >= (LootMaster.db.profile.auto_announce_threshold or 4);
	...
	for candidateID = 1, 40 do repeat
        ...
        self:Debug( format( 'GetMasterLootCandidate(%s, %s) = %s', tostring(LootFrame.selectedSlot), tostring(candidateID), tostring(candidate)) )
